...
function addProduct() {
	var fd = new FormData();
	fd.append("name", getById("product-name").value);
	fd.append("code", getById("product-code").value);
	fd.append("price", getById("product-price").value);
	fd.append("country", getById("product-country").value);
	fd.append("brand_id", getById("product-brand").value);
	fd.append("description", getById("product-description").value);
	var imgList = getByName("product-image");
	for (var i = 0; i < imgList.length; i++) {
		fd.append('image[]', imgList[i].files[0]);
	}
	postRequest("?api=add_product", fd, function (resp) {
...
function deleteProduct(obj) {
	...
	if (!cf) return;
	var fd = new FormData();
	fd.append("product_id", obj.parentElement.parentElement.id);
	postRequest("?api=delete_product", fd, function (resp) {
		// console.log(resp);
		var data = JSON.parse(resp);
		if (data["code"] == "OK") {
